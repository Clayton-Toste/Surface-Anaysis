...
NOTES
    ...
    '''
        ...
        # PDB header as object-state property
        if pymol.internal.check_load_prop('pdb_header',
                object_props, 'object', _self):
            if isinstance(contents, bytes):
                contents = contents.decode()
            m = re.search('^(ATOM  |HETATM|MODEL)', contents, re.M)
            if m is not None:
                try:
                    # this fails for multiplexed loading
                    _self.set_property('pdb_header', contents[:m.start()],
                            oname, state, quiet=quiet)
...
ARGUMENTS
    ...
    reso_high = float: maximum resolution {default: 0, read from file}
        ...
        for maptype in ('fofc', '2fofc'):
            ...
            combinations.append((F, P, maptype))

        if not combinations:
            print(' Fallback: Loading all <amplitude>_<phase> combinations')

            fC = header.getColumnsOfType("F")
            pC = header.getColumnsOfType("P")
            keys = set()
            ...
            for F in fC:
                ...
                for P in pC:
                    ...
                    # same dataset?
                    if len(Fsplit) > 1 and Fsplit[0] != Psplit[0]:
                        continue

                    # short key, only column names (first dataset)
                    key = '%s_%s' % (Fsplit[-1], Psplit[-1])
                    if key in keys:
                        # long key including dataset name
                        key = '%s_%s' % (F, P)
                    else:
                        keys.add(key)

                    combinations.append((F, P, key))

        for F, P, maptype in combinations:
            name = prefix + '.' + maptype
...
DESCRIPTION
    ...
    loadfunctions = {
        'inp': 'pymol.lazyio:load_rigimol_inp',
        ...
        # Incentive
        'vis': 'epymol.vis:load_vis',
        'moe': 'epymol.moe:read_moestr',
        'phypo': 'epymol.ph4:load_phypo',
        'xvol': 'epymol.ph4:load_hypothesis_xvol',
    }
