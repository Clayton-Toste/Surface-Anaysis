...
#                sys.exc_clear()
        ...
        #
                ...
                self.print_suggestions(layer, exc_type, exc_value, tb)
        ...
        def print_suggestions(self, layer, exc_type, exc_value, tb):
            if issubclass(exc_type, ImportError):
                print_install_suggestion(exc_value)
                return

            if not layer.literal_python_fallback:
                return

            # is the last frame the PyMOL command level?
            if tb:
                while tb.tb_next:
                    tb = tb.tb_next
                if layer.sc_path != tb.tb_frame.f_code.co_filename:
                    return

            # the possible command name (missing or misspelled)
            com = layer.input[0]
            if not com.isalnum():
                return

            # typical typo exceptions:
            # zo0m          -> NameError("... 'zo0m' ...")
            # zo0m sele     -> SyntaxError("invalid syntax")
            if issubclass(exc_type, NameError):
                if "'{}'".format(com) not in str(exc_value):
                    return
            elif issubclass(exc_type, SyntaxError):
                if exc_value.msg != 'invalid syntax':
                    return
            else:
                return

            import difflib
            amb = difflib.get_close_matches(com, self.cmd.keyword)
            if not amb:
                return

            colorprinting.suggest('Unrecognized command, did you mean one of these?')
            for a in parsing.list_to_str_list(amb):
                colorprinting.suggest(a)
...
#                                print tuple([pat] + self.cmd.auto_arg[count][full])
    ...
    install_suggestions = {
        'Bio': ['conda install biopython'],
        'collada': ['pip install pycollada'],
        'csb': ['conda install -c speleo3 csb'],
        'lxml': ['conda install lxml'],
        'matplotlib': ['conda install matplotlib'],
        'openbabel': ['conda install -c openbabel openbabel'],
        'OpenGL': ['conda install pyopengl'],
        'psico': ['conda install -c schrodinger pymol-psico'],
        'rdkit': ['conda install -c rdkit rdkit'],
        'scipy': ['conda install scipy'],
        'spacenavigator': ['pip install pywinusb spacenavigator'],
        'spnav': [
            'pip install spnav',
    ...
    def print_install_suggestion(e):
        '''Suggests how to install a missing Python module
        '''
        name = getattr(e, 'name', None) or ''

        if not name:
            m = re.match(r"No module named (.*)", str(e))
            if m is not None:
                name = m.group(1)

        if name not in install_suggestions:
            name = name.split('.')[0]

        suggestions = install_suggestions.get(name, ())

        for suggestion in suggestions:
            colorprinting.suggest('Suggested installation method: %s' % (suggestion,))

        if pymol.IS_MACOS and '/opt/X11/lib/libX11' in str(e):
            colorprinting.suggest('Please install XQuartz (https://www.xquartz.org)')
