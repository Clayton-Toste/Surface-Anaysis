        ...
        r += [
            ...
            [1, 'user properties', [[ 2, 'User Properties:', '' ]] + [
                [ 1, key, [[ 2, 'Palette', '' ]] + [
                    ...
                    for palette in ('rainbow', 'blue white red', 'green red')
                ]] for key in mc.props
            ]],
        ]
    return r
...
def mol_generate(self_cmd, sele):
    ...
    if nstates > 1:
        morphstates.append([1, '1->2', 'cmd.async_(cmd.morph,None,%s,quiet=0)' % (rsele)])
    if nstates > 2:
        morphstates.append([1, '1->%d' % nstates,
            'cmd.async_(cmd.morph,None,%s,None,1,%d,quiet=0)' % (rsele, nstates)])
        label = '1->2->3' if nstates == 3 else '1->2->...->%d' % nstates
        morphstates.append([1, label,
            'cmd.async_(cmd.morph,None,%s,None,0,%d,refinement=0,quiet=1)' % (rsele, nstates)])
        morphstates.append([1, label + '->1',
            'cmd.async_(cmd.morph,None,%s,None,0,refinement=0,quiet=1)' % (rsele)])
    sele_morphs = lambda x: [[ 1, a, 'cmd.async_(cmd.morph,None,%s,%s,quiet=0)' % (rsele, repr(a)) ]
...
def main_pseudoatom_sub(self_cmd,pos, screenpos):
    return [
        ...
        [ 1, 'callout', 'cmd.callout("",'
            '"callout", [%1.7f,%1.7f,%1.7f], [%d,%d], _wiz=1)'
            % (pos + screenpos) ],
        [ 0, ''             , ''                      ],
...
def pick_menu(self_cmd, title, sele2):
    with menucontext(self_cmd, sele2):
        return [[ 2, title     , '' ],
            ...
            [ 0, ''             , ''                      ],
            [ 1, 'edit label', 'wizard labeledit, ' + sele2 ],
            [ 1, 'inspect properties', 'wizard atomprops, ' + sele2 ],
            [ 0, ''             , ''                      ],
