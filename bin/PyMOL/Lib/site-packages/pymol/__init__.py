...
def setup_environ():
    ...
    # find executables (for plugins etc.) in local environment
    if not sys.executable.endswith('MacOS/Python'):
        dirname = os.path.dirname(sys.executable)
        test_paths = [dirname]
        if IS_WINDOWS:
            test_paths.append(os.path.join(dirname, 'Library', 'bin'))
            test_paths.append(os.path.join(dirname, 'Scripts'))
        path = os.getenv('PATH', '').split(os.pathsep)
        for test_path in test_paths:
            if test_path not in path and os.path.isdir(test_path):
                path.insert(0, test_path)
        os.environ['PATH'] = os.pathsep.join(path)

    # "activate" conda environment
    if 'CONDA_DEFAULT_ENV' not in os.environ and \
            os.path.isdir(os.path.join(sys.prefix, 'conda-meta')):
        os.environ['CONDA_DEFAULT_ENV'] = sys.prefix

    # portable conda environment: same variables also patched by
    # "make-portable" script, but the modification might get lost if
    # e.g. updating the conda package for pyopenssl but not openssl
    items = [
        ('SSL_CERT_FILE', sys.prefix + r'\Library\ssl\cacert.pem'),
    ] if IS_WINDOWS else [
        ('SSL_CERT_FILE', sys.prefix + '/ssl/cert.pem'),
        ('QT_XKB_CONFIG_ROOT', sys.prefix + '/lib'),
        ('FONTCONFIG_FILE', sys.prefix + '/etc/fonts/fonts.conf'),
        ('FONTCONFIG_PATH', sys.prefix + '/etc/fonts'),
    ]
    for varname, test_path in items:
        if varname not in os.environ and os.path.exists(test_path):
            os.environ[varname] = test_path
...
def _report_cmd_DLL_problem():
    print("PyMOL failed to start, dumping diagnostics...")
    import pymol.diagnosing as d
    print(d.diagnostics_no_cmd())

    if not IS_WINDOWS:
        return

    # Report in a popup window which pymol._cmd DLL dependency is missing.
    # Only works on Windows 7.
    import platform
    if not int(platform.win32_ver()[0]) < 10:
        d.try_cdll_imports()
        return

    import imp, _ctypes
    filename = imp.find_module('_cmd', [os.path.dirname(__file__)])[1]
    print(filename)
    _ctypes.LoadLibrary(filename)

try:
    import pymol._cmd
except ImportError:
    _report_cmd_DLL_problem()
    raise
