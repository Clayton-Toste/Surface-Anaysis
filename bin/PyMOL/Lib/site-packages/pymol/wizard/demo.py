...
class Demo(Wizard):
    ...
    def __init__(self,name=None,_self=cmd):
        ...
        if name!=None:
            ...
            last_fn = saved.get('last_fn', None)
            if last_fn:
                last_fn(cleanup=1)
                saved['last_fn'] = None
            if hasattr(demo,name):
                ...
                saved['last_fn'] = demo_fn
                t = threading.Thread(target=demo_fn)
                t.setDaemon(1)
                t.start()
                self.message = demo.message_dict.get(name,None)
        
    # for unpickling (PSE restoring)
    def __setstate__(self, state):
        if 'last' in state:
            saved['last_fn'] = getattr(DemoInfo(), state['last'], None)
    ...
    def get_panel(self):
        return [
            ...
            [ 2, 'Volume Rendering', 'replace_wizard demo,volume'],
...
#         self.cmd.zoom("center",12)
    ...
    def volume(self, cleanup=0):
        ...
        try:
            ...
            self.cmd.load("$PYMOL_DATA/demo/il2.pdb")
            self.cmd.hide("everything", "il2")
            self.cmd.show("wire", "il2")
            self.cmd.remove("hydro")
            self.cmd.disable()
            self.cmd.enable("il2")
            self.cmd.map_new("map","gaussian","0.75","il2")
            self.cmd.volume("vol", "map")

            self.cmd.set_view ( (\
                -0.196437865,    0.651553273,    0.732726812,\
                -0.936909735,    0.095652118,   -0.336229950,\
                -0.289160103,   -0.752547383,    0.591656506,\
                -0.000000000,    0.000000000,  -54.603652954,\
                13.530572891,  -13.021860123,   20.089715958,\
                52.442813873,   56.764492035,  -20.000000000 ))

            peak_colors = [ [ "blue", "cyan"],
                            [ "green", "palegreen"],
                            [ "orange", "yellow"],
                            [ "red", "magenta"] ]

            ramp = []
            for peak in range(4, 0, -1):
                ramp[:0] = range(9)  # insert new peak (dummy values)
                for a in range(1, 31):
                    if self.interrupt:
                        return

                    self.cmd.set("suspend_updates", 1)
                    inc = min(peak * a / 60.0, 1.0) ** 3
                    p0 = peak - 0.1
                    p1 = peak
                    p2 = peak + 0.1
                    alpha = inc
                    c0 = peak_colors[peak-1][0]
                    c1 = peak_colors[peak-1][1]
                    ramp[:9] = [
                        p0, c0, 0.0,
                        p1, c1, alpha,
                        p2, c0, 0.0]
                    self.cmd.volume_color("vol", ramp)
                    self.cmd.set("suspend_updates", 0)
                    self.cmd.refresh()
