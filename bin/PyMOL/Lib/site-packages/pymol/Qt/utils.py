...
def conda_ask_install(packagespec, channel=None, msg="", parent=None, url=""):
    ...
    conda_exe = os.getenv('CONDA_EXE') or 'conda'
    args = [conda_exe, 'install', '-y', '-p', sys.prefix, packagespec]

    if channel:
        args += ['-c', channel]

    if channel and '::' not in packagespec:
        friendly = '%s/%s' % (channel, packagespec)
    else:
        friendly = packagespec
    ...
    if url:
        msg += (
            "<b>Disclaimer: {0} is a third-party "
            "software not distributed by Schrodinger. Schrodinger "
            "disclaims all liability associated with the installation "
            "of the library.<br><br>"
            "Before installation, please review {0} license "
            "terms available here:<br><br>"
            '<a href="{1}">{1}</a>'
            "</b><br><br>"
        ).format(packagespec, url)

    msg += 'Do you want to install the conda package "%s" now?' % friendly

    answer = QMB.question(parent, "Install", msg, QMB.Yes | QMB.No, QMB.No)

    if answer != QMB.Yes:
        return False

    env = dict(os.environ)
    env.pop('PYTHONEXECUTABLE', '')  # messes up conda on Mac

    try:
        subprocess.check_call(args, env=env,
                shell=sys.platform.startswith('win'))
    except BaseException as e:
        QMB.critical(parent, "Error", str(e))
        return False

    QMB.information(parent, "Success",
            '"%s" was sucessfully installed' % (friendly))

    return True


def conda_ask_update(packagename, channel=None, parent=None):
    """
    Check if a new version of a conda package is available
    """
    asyncfunc = AsyncFunc(_conda_ask_update1, _conda_ask_update2)

    info = QtWidgets.QMessageBox(parent)
    info.setText("Checking for updates, this can take several minutes...")
    info.setStandardButtons(QtWidgets.QMessageBox.Cancel)
    info.buttonClicked.connect(lambda _: asyncfunc.terminate())

    # pass "asyncfunc" as an additional (unused) argument so that it doesn't
    # go out of scope yet, otherwise the thread might just die
    asyncfunc(packagename, channel, parent, asyncfunc, info)

    info.exec_()

def _conda_ask_update1(packagename, channel, parent, af, infobox):
    import json
    import sys, subprocess, os

    conda_exe = os.getenv('CONDA_EXE') or 'conda'
    args = [conda_exe, 'install', '--dry-run', '--json',
            '-p', sys.prefix, packagename]
    ...
    try:
        process = subprocess.Popen(args, env=env,
                ...
                stderr=subprocess.PIPE, # Windows pythonw fix
    ...
    return result, e_, packagename, channel, parent, af, infobox

def _conda_ask_update2(resulttuple):
    QMB = QtWidgets.QMessageBox

    result, e, packagespec, channel, parent, af, infobox = resulttuple
    ...
    import re
    packagename = re.match(r'([^:]*::)?([^>=<]+)', packagespec).group(2)

    for link in links:
        if link['name'] == packagename:
            packagespec = '{channel}::{name}={version}={build_string}'.format(**link)
            new = link
            break
    else:
        QMB.information(parent, "Up to Date",
                '"%s" is already at the latest version' % (packagename))
        return True

    links = actions.get('UNLINK', [])

    old = {'version': '(none)', 'build_string': '(none)'}
    for link in links:
        if link['name'] == packagename:
            old.update(link)
            break

    msg = "A new version is available: {version} build {build_string}".format(**new)
    msg += "<br>currently installed: {version} build {build_string}".format(**old)

    return conda_ask_install(packagespec, channel, msg, parent)
