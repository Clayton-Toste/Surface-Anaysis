...
def check_load_prop(name, pattern, level='object', _self=cmd):
    if pattern is None:
        pattern = _self.get(
                'load_object_props_default' if level == 'object' else
                'load_atom_props_default')
    return pattern == '*' or name in pattern.split()
...
def _sdof_button(button, _self=cmd):
    _cmd._sdof_button(_self._COb, int(button))
