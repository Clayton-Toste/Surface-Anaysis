...
def _get_pymol_update_spec(cmd):
    v = cmd.get_version()[0]
    v_py = sys.version_info[:2]
    return 'pymol>{}=py{}{}*'.format(v, *v_py)
...
class PyMOLQtGUI(QtWidgets.QMainWindow, pymol._gui.PyMOLDesktopGUI):
    ...
    from .lic_dialog import show_activationdialog
    ...
    def __init__(self):  # noqa
        ...
        colorprinting.ANSI_SUPPORT = True
...
''')
        ...
        # License panel
        from .lic_dialog import setup_activationdialog
        setup_activationdialog(self)

        cmd = self.cmd = self.pymolwidget.cmd

        # feedback browser supports color
        cmd.set('colored_feedback')
        ...
        # Windows file types registration
        if pymol.IS_WINDOWS:
            from pymol import winfiletypes

            menu = self.menudict['Setting']
            menu.addSeparator()
            menu.addAction('Register File Extensions', winfiletypes.extensions_gui)
            menu.addAction('Register with Maestro', winfiletypes.register_with_maestro_gui)

            # show dialog on first launch
            winfiletypes.prompt_user_once(self)
        ...
        # activation dialog menu option
        self.menudict['Help'].addSeparator()
        self.menudict['Help'].addAction(
            'Install new License File', self.show_activationdialog)
        self.menudict['Help'].addAction(
                'Check for Updates', lambda: pymol.Qt.utils.conda_ask_update(
                    _get_pymol_update_spec(cmd), 'schrodinger', parent=self))
        ...
        # space navigator
        if not options.no_spacenav:
            from pymol import spacenavigating
            if spacenavigating.start(self.isActiveWindow, self.cmd, quiet=1):
                print(' Detected SpaceNavigator.')
    ...
    def show_about(self):
        ...
        if 'notice' in info:
            msg.append(info['notice'])

        if 'date' in info:
            msg.append('Expiry date: %s' % (info['date']))
...
def make_pymol_qicon():
    ...
    for size in (16, 32, 128):
        icon.addFile(
            os.path.join(icons_dir, 'icon2_%dx%d.png' % (size, size)),
