...
class NucleicAcidWizard(BioPolymerWizard):
    ...
    HIGHLIGHT_SELE = "(name O3' &! neighbor name P) | (name P &! neighbor name O3')"

    def __init__(self, form, dbl_helix, nuc_type, _self):
        BioPolymerWizard.__init__(self,_self)
...
class _BuilderPanel(QtWidgets.QWidget):
    ...
    def attach_nuc_acid(self, nuc_acid, nuc_type):
        ...
        if len(picked)==1:
            ...
            self.cmd.unpick()
            NucleicAcidWizard(form=self._nuc_acid_prop.dna_form,
                              dbl_helix=self._nuc_acid_prop.dna_dbl_helix,
                              nuc_type=self._nuc_type, _self=self.cmd).toggle(nuc_acid)
