...
def run(input, quiet=0):
    ...
    try:
        ...
        result, stderr = process.communicate(input)
        returncode = process.wait()
    except:
        print("Error: rigimol did not run")
        traceback.print_exc()
    else:
        if stderr and (returncode or not quiet):
            print(stderr.rstrip())
            if returncode == 1:
                # license error
...
def morph(source, target, first=1, last=2, refinement=10, async_=-1, steps=30,
