...
class MOEReader:
    ...
    def next_s_star(self):
        ...
        cnt = self.next_i()
        return [self.next_s() for a in range(cnt)]

    def next_s_count(self, cnt):
        # Observed with a base64 text block in "complex_surface.moe".
        # Not sure if this operation counts lines or words and if
        # lines/words are supposed to be concatenated.
        return [self.force_next_word() for a in range(cnt)]
    ...
    def next_paraTags(self,count):
        ...
        while count>0:
            ...
            if _s_count_re.match(code) is not None:
                result[tag] = self.next_s_count(int(code[1:]))
                continue
    ...
    def next_paraTable_slow(self,count,columns):
        methods = [x[2] for x in columns]
        return [[m() for m in methods] for x in range(count)]
        ...
        if (count>100) and len(columns):
            if (columns[0][3]==1) and (columns[-1][3]==1): # one and only one input in the table
                if columns[0][1]=='i': # simple table of ints (probably IDs)
                    uniform_part = [c[2]() for c in columns[1:]]
                    variable_part = self.fast_i_star(count)
                    return [[v] + uniform_part for v in variable_part]
    ...
    def _txf_ixrrrrrr(self,value):  # isn't there something like a "group" operation in Python?
        ...
        if self._txf_cnt.pop(0):
            row = self._txf_row
            self._txf_result.append([int(row[0],16)] + list(map(float,row[1:7])))
            self._txf_row = []
    ...
    def fast_ixrrrrrr(self,count):
        ...
        self._txf_row = []
        self._txf_cnt = [0,0,0,0,0,0,1]
        list(map(self._txf_ixrrrrrr, queue))
    ...
    def fast_ii(self,count):
        ...
        self._txf_result = []
        self._txf_prior = None
        list(map(self._txf_ii, queue))
        result = self._txf_result
        del self._txf_result
        return result

    def next_gvertex(self,count,columns):
        test_columns = [(x[1],x[3]) for x in columns]
...
#        print count,map(lambda x:(x[0],x[1],x[3]),columns)
    ...
    def _kw_graphics(self):
        ...
        if fields != [ 'tag', 't', 'value', '*' ]:
            raise MOEReaderException("unknown pharmacophore format")
        if 'graphics' not in self.system:
    ...
    def appendFromStr(self,moe_st):
        ...
        line = moe_st.split('\n')
        self.line = line
        self.line_1st_ch = [x[0:1] for x in line]
        while 1:
            self.word = []
            try:
