...
ARGUMENTS
    ...
    try:
        colors = data[u'Colors of Vertices'].value.reshape((-1,3)).tolist()
    except KeyError:
        colors = None
    ...
    if style == 1:
        ...
        patches = data[u'Lines'].value.reshape((-1,2))
    else:
        obj.append(cgo.TRIANGLES)

    color = None

    for i in patches.flat:
        if colors is not None and colors[i] != color:
            color = colors[i]
            obj.append(cgo.COLOR)
            obj.extend(color)

        obj.append(cgo.NORMAL)
        obj.extend(normals[i].tolist())
        obj.append(cgo.VERTEX)
        obj.extend(vertices[i].tolist())
...
def load_vis(filename, object='', mimic=1, quiet=1, _self=cmd):
    '''
DESCRIPTION
...
SEE ALSO
    ...
    return func(data, object, mimic, quiet, _self)
