...
def load_hypothesis_xyz(filename, name='', ref='', zoom=-1):
    '''
    ...
    pymol.cmd.load_cgo(obj, name, zoom=zoom)
...
def load_hypothesis_xvol(filename, name=''):
    ...
    try:
        ...
        cols_xyz = [table.keys.index(n) for n in ['r_m_phase_x', 'r_m_phase_y', 'r_m_phase_z']]
...
def load_phypo(filename, prefix, zoom, mimic, atom_props, _self):
    '''
    Load Phase "phypo" files.
    '''
    import base64
    import tempfile
    from chempy import Atom, models
    ...
    def get_blob(key):
        s = _self.get_property(key, name)
        return base64.decodestring(s.encode()).decode() if s else ''
    ...
    def site_label_gen(txt):
        for line in txt.splitlines():
            a = line.split()
            yield a[1] + str(int(a[0]) + 1)

    def load_sites(oname):
        sites_blob = get_blob('s_phasehypo_Sites')
        if not sites_blob:
            return

        _self.alter(name, 'name = _next()',
                space={'_next': lambda it=site_label_gen(sites_blob): next(it)})
        _self.label(name, 'name')

        with open(xyzfile1, 'w') as handle:
            handle.write(sites_blob)

        with open(xyzfile2, 'w') as handle:
            handle.write(get_blob('s_phasehypo_SitesWithProj'))

        load_hypothesis_xyz(xyzfile1, oname, xyzfile2, zoom=0)

    # load as Maestro file with all object-level properties
    _self.load(filename, prefix, format='mae', zoom=zoom, mimic=mimic,
            multiplex=1, object_props='*', atom_props=atom_props)

    # get names of loaded entries
    names = _self.get_object_list(prefix + '*')

    xyzfile1 = tempfile.mktemp('1.xyz')
    xyzfile2 = tempfile.mktemp('2.xyz')

    try:
        for name in names:
            # look for entries with hypothesis meta data
            if _self.get_property('s_phasehypo_HypoRole', name) != 'hypo':
                continue

            _self.set('retain_order', 1, name)
            load_sites(name + '_cgo')
            load_xvol(name + '_xvol')
    finally:
        try:
            os.unlink(xyzfile1)
            os.unlink(xyzfile2)
