...
def typecast(c, x):
    ...
    try:
        if typeprefix == 'r_':
            return float(x)
        if typeprefix == 'i_' or typeprefix == 'b_':
            return int(x)
        return x
    except ValueError:
        print(" Warning: type casting failed:", c)
        return x
...
def set_molecule_property(code, x,a):
    a.molecule_properties.append([code, typecast(code, x)])
...
def default_atom_handler(c,x,a):
    a.atom_properties[c] = typecast(c, x)
...
def hanlder_i_m_ribbon_color(c, x, a):
    try:
        color = _color_dict[int(x)]
    except KeyError:
        return

    if getattr(a, 'i_m_color', None) == color:
        return

    a.ribbon_trgb = color
    a.cartoon_trgb = color

def hanlder_i_m_label_color(c, x, a):
    try:
        color = _color_dict[int(x)]
    except KeyError:
        return
        ...
        if len(self.stack):
            ...
            if isinstance(tag, basestring):
                if tag[0:7]=='m_atom[' and tag[-1:]==']':
                    ...
                    bond_list = [x for x in cur_bond_block if x.index[0]<x.index[1]]
